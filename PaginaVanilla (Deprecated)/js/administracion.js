$(document).ready(() => {
  ...
  let update = "http://175.10.0.166:8080/api/update"
      ...
      .then((res)=>{
          ...
          res.data.forEach(data=> {
            ...
            <li class=" text-center text-lg p-2">Variador ${data.modelo_equipo}</li>
            ...
            <button type="button" class="botonMostrar ${color} text p-2 mb-2 rounded-xl text-white" data-activado="${data.estado_equipo}" data-id="${data._id}">${estado}</button>
  ...
  const handleClickMostrar = () => {
    $(".botonMostrar").click((event) => {
      ...
      //botonClickeado.attr("data-activado", !valorActualConvertido); //cambia el data-activado a true o false
      if (valorActual == 0 || valorActual == 2 || valorActual == 3) {
        ...
        .then(()=>{
          ...
          botonClickeado.text("Haciendo");
          event.currentTarget.setAttribute('data-activado',  '1');
          })
        .catch(()=>{
          console.log("mal")
        })
        ...
        .then(()=>{
          ...
          event.currentTarget.setAttribute('data-activado',  '2');
        })
        .catch(()=>{
          console.log("mal")
        })
 ...
 const handleClickFaker= () => {
    $(".cargaFaker").click((event)=>{
